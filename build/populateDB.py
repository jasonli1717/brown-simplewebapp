...
def makeAuthorList(article_table_obj_list):
    ...
    for article in article_table_obj_list:
        for auth in article.author_list:
            ...
            #    aname = (auth.lastname, auth.firstname, auth.authid)
            #    authortab.append(aname)
        stdout.write('\r' + "processing author no. " + str(len(name_list)))
        stdout.flush()
    return authortab
...
def processChunk (chunk):
    data = queryPubmed.fetchDetails(chunk)
...
if __name__ == '__main__':
    ...
    #this takes about 20 minutes because of the efetch limit of about 200 retrievals
    for chunk in chunks:
        refined_chunk=processChunk(chunk)
        papers = papers + refined_chunk
        stdout.write(str(len(papers) * 100 / len(id_list)) + '% of entries fetched' + '\n')
        stdout.flush()


   # data = queryPubmed.fetch_details(chunks[1])
    #papers = query.Pubmed.extractEntry(data)
    #print len(papers)
